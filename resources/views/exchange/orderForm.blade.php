		...
		<div class="form-group">
			...
			<div class="form-group row">
				...
				<div class="col-sm-6 text-right">
					<small>
						@if(Auth::check())
							<span id="buy-balance" class="balance clickable">{{ $balance[$coin2->symbol] }}</span> {{ $coin2->symbol}}
						@else
							<span>-</span> {{ $coin2->symbol}}
						@endif
					</small>
				</div>
			</div>
		...
		<div class="form-group">
			...
			<div class="form-group row">
				...
				<div class="col-sm-6 text-right">
					<small>
						@if(Auth::check())
							<span id="sell-balance" class="balance clickable">{{ $balance[$coin1->symbol] }}</span> {{ $coin1->symbol}}
						@else
							<span>-</span> {{ $coin1->symbol}}
						@endif
					</small>
				</div>
			</div>
