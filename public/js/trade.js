...
function updateUserOrderHistory(data) {
    ...
    data.forEach(function (order) {
        ...
        row.hide();
        $("#table-user-history tbody").prepend(row);
        row.fadeIn('slow');
    });
}
...
function updateMarketHistory(data) {
    ...
    data.forEach(function (order) {
        ...
        if (totalRows == 0) {
            ...
            tbody.prepend('<tr style="display:none"><td></td><td></td><td></td></tr>');
        }
...
function updateOrderBook(data) {
    ...
    $('#table-order-book tbody').each(function (index) {
        var tbody = $(this);
        var book = tbody.attr('data-key-name');
        ...
        for (i in data[book]) {
            ...
            if (totalRows == 0) {
                console.log('prepending!!! ' + order);
                mirrorBook[book][order.price] = '';
                tbody.prepend(getBookRow(order.price, order.amount, order.total));
                continue;
            }
            ...
            bodyRows.each(function (rowIndex) {
                ...
                if (mirrorBook[book][order.price] == '') {
                    return true;
                }
